...
class UpdateActorModal extends Component {
  componentDidMount() {
  ...
  submitForm(event) {
    ...
    this.props.update({ ...this.props.actor, name, description });
    this.props.close();
  }

  render() {
    const { close, actor } = this.props;
        ...
        <form onSubmit={this.submitForm.bind(this)} ref={form => { this.form = form }}>
          ...
          <div className='modal-card'>
            <header className='modal-card-head'>
              <p className='modal-card-title'>Updating actor</p>
